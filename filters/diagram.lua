...
local plantuml = {
    line_comment_start = [[']],
    mime_types = mime_types_set {'svg', 'png', 'pdf'},
    ...
    compile = function(self, code)
        ...
        return with_temporary_directory("diagram", function(tmpdir)
            return with_working_directory(tmpdir, function()
                ...
                pipe(self.execpath or 'mmdc', {"-p", home .. "/.local/share/md2pdf/filters/puppeteer-config.json",
        ...
        return with_temporary_directory("tikz", function(tmpdir)
            return with_working_directory(tmpdir, function()
                ...
                local meta = {
                    ...
                    ['additional-packages'] = {pandoc.RawInline('latex',
                        stringify(user_opts['additional-packages'] or ''))}
                }
                local tex_code = pandoc.write(pandoc.Pandoc({pandoc.RawBlock('latex', src)}, meta), 'latex', {
                    template = tikz_template
                })
                write_file(tikz_file, tex_code)

                -- Execute the LaTeX compiler:
                local success, result = pcall(pipe, self.execpath or 'pdflatex',
                    {'-interaction=nonstopmode', '-output-directory', tmpdir, tikz_file}, '')
                if not success then
                    warn(string.format("The call\n%s\nfailed with error code %s. Output:\n%s", result.command,
                        result.error_code, result.output))
                end
...
local pikchr = {
    line_comment_start = '#',
    mime_types = mime_types_set {'svg'},
    compile = function(self, code)
        local mime_type = self.mime_type or 'image/svg+xml'
        return with_temporary_directory("diagram", function(tmpdir)
            return with_working_directory(tmpdir, function()
                local infile = 'diagram.pikchr'
                write_file(infile, code)
                return pipe(self.execpath or 'pikchr-cli', {infile}, ''), mime_type
            end)
        end)
    end
}
...
local function format_options(name)
    ...
    local preferred_mime_types = is_office_format and pandoc.List {'image/png', 'application/pdf'} or
                                     pandoc.List {'application/pdf', 'image/png'}
        ...
        best_mime_type = function(self, supported_mime_types, requested)
            return self.preferred_mime_types:find_if(function(preferred)
                return supported_mime_types[preferred] and
                           (not requested or (pandoc.utils.type(requested) == 'List' and requested:includes(preferred)) or
                               (pandoc.utils.type(requested) == 'table' and requested[preferred]) or
...
local pdf2svg = function(imgdata)
    ...
    local args = {'--export-type=svg', '--export-plain-svg', '--export-filename=-', pdf_file}
...
local function code_to_figure(conf)
    return function(block)
        ...
        if conf.cache then
            imgdata, imgtype = get_cached_image(pandoc.sha1(block.text),
